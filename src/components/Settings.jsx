import { useState } from 'react';

const Settings = ({ 
  themes, setThemes, 
  bingoMissions, setBingoMissions, 
  valueThemes, setValueThemes 
}) => {
  const [newTheme, setNewTheme] = useState("");
  const [newBingo, setNewBingo] = useState("");
  const [newValue, setNewValue] = useState("");

  // --- トークテーマ操作 ---
  const addTheme = () => {
    if (!newTheme.trim()) return;
    setThemes([...themes, { text: newTheme, enabled: true }]);
    setNewTheme("");
  };
  const toggleTheme = (index) => {
    const updated = [...themes];
    updated[index].enabled = !updated[index].enabled;
    setThemes(updated);
  };
  const deleteTheme = (index) => {
    if (window.confirm("削除しますか？")) setThemes(themes.filter((_, i) => i !== index));
  };

  // --- ビンゴ操作 ---
  const addBingo = () => {
    if (!newBingo.trim()) return;
    setBingoMissions([...bingoMissions, { id: Date.now(), text: newBingo, checked: false }]);
    setNewBingo("");
  };
  const deleteBingo = (id) => {
    if (window.confirm("削除しますか？")) setBingoMissions(bingoMissions.filter(m => m.id !== id));
  };

  // --- 価値観トーク操作 ---
  const addValue = () => {
    if (!newValue.trim()) return;
    setValueThemes([...valueThemes, newValue]);
    setNewValue("");
  };
  const deleteValue = (index) => {
    if (window.confirm("削除しますか？")) setValueThemes(valueThemes.filter((_, i) => i !== index));
  };

  return (
    <div style={{ padding: '20px', width: '100%', maxWidth: '500px' }}>
      
      {/* 1. トークテーマ管理 */}
      <section style={sectionStyle}>
        <h3 style={titleStyle}>💬 トークテーマ管理</h3>
        <div style={inputGroupStyle}>
          <input value={newTheme} onChange={(e) => setNewTheme(e.target.value)} placeholder="新お題..." style={inputStyle} />
          <button onClick={addTheme} style={addButtonStyle}>追加</button>
        </div>
        <div style={listContainerStyle}>
          {themes.map((t, i) => (
            <div key={i} style={{...listItemStyle, opacity: t.enabled ? 1 : 0.4}}>
              <input type="checkbox" checked={t.enabled} onChange={() => toggleTheme(i)} />
              <span style={{flex: 1, marginLeft: '10px'}}>{t.text}</span>
              <button onClick={() => deleteTheme(i)} style={deleteButtonStyle}>削除</button>
            </div>
          ))}
        </div>
      </section>

      {/* 2. ビンゴ管理 */}
      <section style={sectionStyle}>
        <h3 style={titleStyle}>🎯 ビンゴのミッション</h3>
        <div style={inputGroupStyle}>
          <input value={newBingo} onChange={(e) => setNewBingo(e.target.value)} placeholder="新ミッション..." style={inputStyle} />
          <button onClick={addBingo} style={addButtonStyle}>追加</button>
        </div>
        <div style={listContainerStyle}>
          {bingoMissions.map((m) => (
            <div key={m.id} style={listItemStyle}>
              <span style={{flex: 1}}>{m.text}</span>
              <button onClick={() => deleteBingo(m.id)} style={deleteButtonStyle}>削除</button>
            </div>
          ))}
        </div>
      </section>

      {/* 3. 価値観トーク管理 */}
      <section style={sectionStyle}>
        <h3 style={titleStyle}>🃏 価値観トークのお題</h3>
        <div style={inputGroupStyle}>
          <input value={newValue} onChange={(e) => setNewValue(e.target.value)} placeholder="新テーマ..." style={inputStyle} />
          <button onClick={addValue} style={addButtonStyle}>追加</button>
        </div>
        <div style={listContainerStyle}>
          {valueThemes.map((v, i) => (
            <div key={i} style={listItemStyle}>
              <span style={{flex: 1}}>{v}</span>
              <button onClick={() => deleteValue(i)} style={deleteButtonStyle}>削除</button>
            </div>
          ))}
        </div>
      </section>

    </div>
  );
};

// スタイル定義
const sectionStyle = { marginBottom: '40px' };
const titleStyle = { borderLeft: '4px solid #8c7b75', paddingLeft: '10px', marginBottom: '15px', fontSize: '1.1rem' };
const inputGroupStyle = { display: 'flex', gap: '5px', marginBottom: '10px' };
const inputStyle = { flex: 1, padding: '10px', borderRadius: '8px', border: '1px solid #ddd' };
const addButtonStyle = { padding: '0 15px', background: '#8c7b75', color: '#fff', border: 'none', borderRadius: '8px' };
const listContainerStyle = { background: '#f9f9f9', borderRadius: '12px', padding: '10px', maxHeight: '250px', overflowY: 'auto' };
const listItemStyle = { display: 'flex', alignItems: 'center', padding: '8px 0', borderBottom: '1px solid #eee' };
const deleteButtonStyle = { background: 'none', border: 'none', color: '#ff4d4d', cursor: 'pointer' };

export default Settings;